...
def add_watermark(image_path, base64_str):
    if not base64_str or "YOUR_BASE64" in base64_str: return
    try:
...
def get_economic_intel():
    """Robust Forex Factory Scraper with institutional headers."""
    url = "https://nfs.forexfactory.com/ff_calendar_thisweek.json"
    tz_est = pytz.timezone('US/Eastern')
    now = datetime.now(tz_est)
    
    # Institutional Headers to bypass 403 blocks
    headers = {
    ...
    try:
        ...
        if response.status_code != 200:
            return "Forex Factory Restricted (403).", 0x2ecc71
            
        data = response.json()
        today_reds, future_reds = [], []
        
        for event in data:
            # Filter for USD High Impact (Red Folders)
        ...
        if today_reds:
            return "\n".join(today_reds), 0xe74c3c # Red Alert
        
        if future_reds:
            future_reds.sort(key=lambda x: x[0])
            next_dt, next_name = future_reds[0]
            day_label = "Tomorrow" if (next_dt.date() - now.date()).days == 1 else next_dt.strftime('%A')
            return f"No High Impact Today.\n**Next Intel:** {next_name} ({day_label} @ {next_dt.strftime('%I:%M %p')})", 0x2ecc71
            
        return "No High Impact Scheduled.", 0x2ecc71
    except Exception as e:
        return f"Forex Factory Sync Offline.", 0x2ecc71

# --- [REST OF YOUR NEWS AND DATA FUNCTIONS REMAIN THE SAME] ---

def main():
    webhook = os.getenv("DISCORD_WEBHOOK_URL")
    finnhub_key = os.getenv("FINNHUB_KEY") # Still here if needed for news
    current_est = format_est_time()
    
    # RESTORED FOREX FACTORY INTEL
    eco_intel, embed_color = get_economic_intel()
    
    # Briefing (Uses your existing news logic if you kept it)
    briefing = "Direct intelligence feed from UWS institutional wires."
