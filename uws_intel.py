...
def get_forex_factory_intel():
    ...
    try:
        ...
        for event in data:
            # Filter: USD only and High Impact only
            if event.get('currency') == 'USD' and event.get('impact') == 'High':
                # Convert FF ISO string to EST
        ...
        if future_reds:
            ...
            next_day = next_dt.strftime('%A @ %I:%M %p')
            return f"No High Impact Today.\n**Next Intel:** {next_name} ({next_day})", 0x2ecc71 # Safe Green
            
        return "No High Impact Scheduled.", 0x2ecc71
    except Exception as e:
        return f"Forex Factory Sync Failed: {str(e)}", 0x2ecc71
...
def main():
    ...
    embeds = [{
        ...
        "fields": [
            ...
            {"name": "\u200B", "value": "\u200B", "inline": False},
            {"name": "🗞️ Market Briefing", "value": "Direct feed from UWS institutional wires.", "inline": False}
        ],
