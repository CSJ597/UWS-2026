...
def get_economic_intel(api_key):
    ...
    try:
        ...
        for event in calendar:
            if event.get('impact') == 3 and event.get('country') == 'US':
                e_date_raw = event.get('date', '')
                ...
                if e_dt.date() == now.date():
                    diff = e_dt - now
                    m_left = int(diff.total_seconds() // 60)
                    timer = f" (In {m_left}m)" if m_left > 0 else " (JUST RELEASED)" if m_left > -60 else ""
                    today_events.append(f"🚩 **{event.get('event')}**{timer}")
                elif e_dt > now:
                    future_events.append(e_dt)

        if today_events: return "\n".join(today_events), 0xe74c3c
        if future_events:
            next_ev = min(future_events)
