...
import requests as discord_requests # Standard requests for Discord
from curl_cffi import requests as news_requests # Anti-bot for news
import json
import base64
import sys
import csv
...
# --- 🏦 LOGO CONFIG ---
# Paste your actual Base64 string between the triple quotes
...
def add_watermark(image_path, base64_str):
    ...
    try:
        # Clean string and fix padding (solves "multiple of 4" error)
...
def get_forex_factory_intel():
    """Fetches High Impact USD events. Tries JSON, falls back to stable CSV."""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if future_reds:
                ...
                nxt = future_reds[0]
                return f"No News Today. Next: {nxt[1]} ({nxt[0].strftime('%a @ %I:%M %p')})", 0x2ecc71
    except: pass

    # Fallback to Stable CSV CDN
    try:
...
def get_finnhub_briefing(api_key):
    """Restores the Gold and Nasdaq news briefings."""
    if not api_key: return "Market stream offline."
...
def get_precision_data(ticker):
    """Calculates algorithmic UWS levels with safety guards."""
...
def main():
    print("🚀 Initializing Battle Prep...")
    ...
    for i, asset in enumerate(assets):
        ...
        if df is not None:
            ...
            for label, price in lvls.items():
                axlist[0].text(len(df) + 2.5, price, f"{round(price, 2)} - {label}", 
                               color='#C0C0C0', fontsize=8, va='center', fontweight='bold')
    ...
    if webhook:
        # Final post uses standard requests to ensure image delivery
